...
def gather_metadata(item):
    ...
    transcript = "\n".join([' '.join(re.compile(r'[\n\r\t]').sub(" ",t.find('p').contents[0]).split()) for t in transcript_elem])
    ...
    return {
            ...
            "duration": val_check(current,"duration"),
            "event": val_check(current,"event"),
            "languages": val_check(current,"languages"),
...
df.to_csv("ted-talks/ted-talks-scraped.csv", index=False)
